...
async def set_lang(message: types.Message, state: FSMContext):
    ...
    kb = types.ReplyKeyboardMarkup(keyboard=[[types.KeyboardButton(text=MESSAGES[lang]['btn_pres']), types.KeyboardButton(text=MESSAGES[lang]['btn_kurs'])],[types.KeyboardButton(text=MESSAGES[lang]['btn_sam'])]], resize_keyboard=True)
    await message.answer(MESSAGES[lang]['services'], reply_markup=kb)

@dp.message(F.text.in_([MESSAGES[l][k] for l in MESSAGES for k in ['btn_pres', 'btn_kurs', 'btn_sam']]))
...
async def show_check(message: types.Message, state: FSMContext):
    ...
    text = MESSAGES[data['lang']]['check'].format(service=data['service'], topic=data['topic'], pages=message.text)
    await message.answer(text, reply_markup=get_confirm_kb(data['lang']))
    await state.set_state(Order.confirming)

@dp.message(Order.confirming, F.text.in_([MESSAGES[l]['confirm_btn'] for l in MESSAGES]))
async def send_payment(message: types.Message, state: FSMContext):
    data = await state.get_data()
    admin_text = f"✅ ЗАКАЗ: {data['service']}\nТема: {data['topic']}\nЛистов: {data['pages']}\nОт: @{message.from_user.username}"
    await bot.send_message(ADMIN_ID, admin_text)
    pay_text = MESSAGES[data['lang']]['pay'].format(price=data['price'], card=CARD_NUMBER)
